...
## Day 6. 새 클론 검증과 문서 대조
#### 2026-09-17 (목)

### 수행 내용

- 레포지토리를 clone해 빌드 및 테스트 진행
- README 절차 그대로 실행. Mock과 app 기동, Swagger, H2 콘솔, 재고 API 장애/무응답 재현
- 매핑 동기화 실패 재현
  - A 목록 API를 고장낸 채 재시작
  - A 실패, B 성공, 앱 정상 기동 확인
  - 검색에 A는 NO_MAPPING
  - 복귀 후 재시도에서 A 매핑 생성
  - 로그로 전 과정 확인
- README에 netty DNS 로그 안내, 매핑 동기화 실패 재현 절차 추가
- ADR 0004에 재시도 시각이 틱 간격만큼 늦어질 수 있다는 비용 추가

### 의사결정

- macOS netty DNS ERROR 로그는 의존성으로 없애지 않고 README에 무관함을 안내. 실행 환경마다 다른 네이티브 분류자를 고르는 부담이 더 큼
- 재시도 첫 시각의 30~60초 편차는 코드를 고치지 않고 ADR 0004 비용으로 기록

### AI 활용

| 물음 | 답                                        | 판단                      |
|---|------------------------------------------|-------------------------|
| 오늘 문서 초안 (README 재현 절차, JOURNAL) | 검증 로그를 바탕으로 초안                           | 직접 재현한 결과와 대조해 수용       |
| netty ERROR 로그의 원인 | macOS 전용 DNS 라이브러리 미탑재 알림                | README 안내로 작성           |
| 첫 재시도가 30초가 아니라 60초 뒤에 온 이유 | 틱 경계 때문. 틱이 "시각이 지난 공급사"만 고르므로 다음 틱으로 밀림 | 코드 수정 대신 ADR 비용으로 기록    |

### 막힌 점

- 재시도 첫 시각이 문서(30초)와 실제(60초) 다름. 틱 간격과 재시도 초기값이 같아 생기는 경계 문제임을 확인
